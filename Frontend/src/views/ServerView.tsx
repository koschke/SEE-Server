...
import { faArrowLeft, faDownload, faEye, faPlay, faStop, faClipboard, faTrash, faX } from "@fortawesome/free-solid-svg-icons";
...
function ServerView() {
     ...
     */
    async function uploadFile(file: SeeFile) {
        const form = new FormData();
        form.append("id", serverID);
        form.append("projectType", file.projectType);
        form.append("file", file._localfile);
                ...
                : <Card sx={{ marginTop: "2em", borderRadius: "25px", height: "calc(100% - 100px)", overflow: "auto" }}>
                    <CardContent sx={{ height: "calc(100% - 3em)" }}>
                        <Stack direction="column" spacing={2} height={"100%"}>
                            <Stack direction="row" spacing={2}>
                                <Box width={140} height={140}>
                                    <Card sx={{ width: "100%", height: "100%" }}>
                                        <Avatar width={140} height={140} avatarSeed={server.avatarSeed}
                                ...
                                <Stack direction="column" spacing={1}>
                                    ...
                                    {server.status == "ONLINE"
                                        ? <Typography>Online
                                            since: {new Date(server.startTime * 1000).toLocaleDateString()} {new Date(server.startTime * 1000).toLocaleTimeString()}</Typography>
                                    ...
                                    <Stack direction="row">
                                        ...
                                        <Typography
                                            sx={{ fontFamily: "monospace" }}>{server.containerAddress}:{server.containerPort}</Typography>
                                        <IconButton
                                            size="small"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                    ...
                                    {server.serverPassword &&
                                        <Stack direction="row">
                                            ...
                                            <Typography
                                                sx={{ fontFamily: "monospace" }}>{showPassword ? server.serverPassword : server.serverPassword.replace(/./g, "\u25CF")}</Typography>
                            ...
                            <div>
                                <Typography variant="h6" sx={{ marginBottom: "6pt", marginTop: "6pt" }}>Project
                                    Files</Typography>
                                <Card sx={{
                                    borderRadius: "25px",
                                    ...
                                    <CardContent>
                                        <Button variant="contained" color="primary" style={{ marginBottom: "13px" }}
                                            onClick={() => setFileTypeModalOpen(true)}>Add Code City</Button>
                                        ...
                                        {files && files.length > 0 ?
                                            (<List>
                                                                    ...
                                                                    projectFile.name == undefined ?
                                                                        <MuiFileInput
                                                                            ...
                                                                            onChange={async (value) => {
                                                                                setFiles((files) => files?.map((item, itemIdx) => itemIdx === idx ? {
                                                                                    ...item,
                                                                                    _localfile: value
                                                                                } as SeeFile : item));
                                                                                projectFile._localfile = value!;
                                                                                await uploadFile(projectFile);
                                                                        ...
                                                                        <><Grid item xs={5}>
                                                                            <ListItemText>
                                                                                <Typography sx={{
                                                                                    fontWeight: "bold",
                                                                                    marginLeft: "2pt"
                                                                                }}>{projectFile.name}</Typography>
                                                                            </ListItemText>
                                                                        </Grid><Grid item xs={4}>
                                                                                <ListItemText>
                                                                                    <Typography
                                                                                        sx={{ fontStyle: "italic" }}>{ProjectTypeUtils.getLabel(projectFile.projectType)}</Typography>
                                                                                </ListItemText>
                                                                            </Grid><Grid item xs={2}
                                                                                sx={{ textAlign: "right" }}>
                                                                                <ListItemText>
                                                                                ...
                                                                                sx={{ textAlign: "right" }}>
                                                                                <ListItemText>
                                                                                    <a href={axiosInstance.getUri() + "file/download?id=" + projectFile.id}
                                                                                        download={projectFile.name}
                                                ...
                                                // When there are no files uploaded yet.
                                                <Typography variant="h6" sx={{ marginTop: "1em" }}>No files
                                                    uploaded.</Typography>
                                            )
                                        }
                                    </CardContent>
